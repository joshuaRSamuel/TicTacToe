...
/**********************************************************************************************************************************************************
	...
	Description: Program that works with pic18fk22 platform to create a  3x3 tictactoe gameboard interface using led's, pushButtons and a live terminal display
		...
		Mbed platform includes pushbuttons which make player placement and sent it serially to the pic.
...
#define BUFSIZE         30
#define INCRPRESS       0x2
#define DECRPRESS       0x1
#define ENTERPRESS      0x4
#define BOTHPRESS       0xC

Serial pc(USBTX, USBRX);         //serial connection between mbed and usb terminal 
Serial pic(p28, p27);            //serial connection between mbed and pic


DigitalIn pbMode(p11);
DigitalIn pbEnter(p12);
DigitalIn pbIncrement(p13);
DigitalIn pbDecrement(p14);

Timer gameTime;
Ticker secondCounter;
...
char gameReset[STATUSSIZE];
char stringRcvd = FALSE;
char messageRcvd = FALSE;
...
void initializeTttSys()
{
	...
	ttt285.statusFlag[5] = FALSE;            //setting DRAW condition to false
...
Date:       05/12/2020
Modified:   08/12/2020
Desc:       function that receives transmitted byte and stores into rxBuf until entire command string is recieved
Input:      None
Returns:    None
...
void receiveByte()
{
	...
	if (pic.readable())
	{
		...
		{
			rxBuf[insert + 1] = 0x00;
			pc.printf("\e[25;0Hreached receive");
			stringRcvd = TRUE;
		}
		rxBuf[insert] = hold;
		insert++;
...
Date:       18/11/2020
Modified:   None
Desc:       Program for validating cheksum of received and transmitted message
Input:      None
Returns:    None
**********************************************************************************/
char validateSentence(char* vPtr)
{
	char rcs = 0, ncs = 0;               //variables for new and old checkSum
	char csFlag = FALSE;            //flag to denote completed checkSum
	...
	while (csFlag != TRUE)
	{
		...
		if (*(vPtr + count) == ',')
		{
			*(vPtr + count) = 0x00;
			...
			pc.printf("\e[26;0Hreached validate");
			csFlag = TRUE;
		}
		count--;
	}
	...
	if (ncs == rcs)
	{
		return TRUE;                  //right string is received
	}
	else
	{
...
Date:       18/11/2020
Modified:   None
Desc:       Program for breaking up received message and storing into tokens
Input:      None
Returns:    None
**********************************************************************************/
void parseSentence(char* parsePtr)
{
	...
	pc.printf("\e[27;0Hreached parse");
}
...
Date:       05/12/2020
Modified:   None
Desc:       Function for inputting X or O depending on command string rcvd from pic
Input:      int rowNumber, int colNumber
Returns:    None
**********************************************************************************/
...
}   //eo characterSelection

/*** executeSentence::************************************************************
Author:     JMichael
Date:       18/11/2020
Modified:   None
Desc:       Program for executing functions based on Command statement.
Input:      None
Returns:    None
**********************************************************************************/
...
{
	pc.printf("\e[28;0Hreached execute");
	if (atoi(tokens[ADDYTO]) == CONTRADDR)
	{
		if (atoi(tokens[ADDYFM]) == TTTADDY)
		{
			...
			if (strcmp(tokens[CMDSTMNT], gameEnd) == 0)
			{
				if (*(tokens[RESULT]) == 'W')
				{
					if (*(tokens[PLAYER]) == 'X')
					{
						...
						ttt285.gameTime.min = atoi(tokens[5]);
						ttt285.gameTime.min = ttt285.gameTime.min >> 4;
						ttt285.gameTime.sec = atoi(tokens[5]);
...
Date:       03/10/2020
Modified:
Desc:       increments the row/column number from 0 to 2 and rolls over back to 0 when pressed
Input:      None
Returns:    None
**************************************************************************************/
void increment()
{
	...
	if (ttt285.selection == TRUE)        //condition for row selection 
	{
		ttt285.rowSelect++;
		if (ttt285.rowSelect > 2)
		{
			ttt285.rowSelect = 0;
		}
	}
	else if (ttt285.selection == FALSE)      //condition for column selection 
	{
		ttt285.columnSelect++;
		if (ttt285.columnSelect > 2)
		{
			ttt285.columnSelect = 0;
		}
	}
...
Date:       03/10/2020
Modified:
Desc:       decrements the row/column number from 2 to 0 and rolls over back to 2 when pressed
Input:      None
Returns:    None
**************************************************************************************/
void decrement()
{
	if (ttt285.selection == TRUE)
	{
		ttt285.rowSelect--;
		if (ttt285.rowSelect < 0)
		{
			ttt285.rowSelect = 2;
		}
	}
	else if (ttt285.selection == FALSE)
	{
		ttt285.columnSelect--;
		if (ttt285.columnSelect < 0)
		{
			ttt285.columnSelect = 2;
		}
	}
} // eo increment 
...
Date:       03/10/2020
Modified:   07/11/2020
Desc:       function that takes the current row or column number when pressed and prepares command string indicating player placement
Input:      None
Returns:    None
**********************************************************************************/
void enter()
{
	if (ttt285.selection == TRUE)
	{
		ttt285.selection = FALSE;
	}
	else if (ttt285.selection == FALSE)
	{
		if (ttt285.gameTtt.turnCounter % 2 == 0)
		{
			ttt285.gameTtt.playerNo = TRUE;
			ttt285.gameTtt.ticTacToe[ttt285.rowSelect][ttt285.columnSelect] = 'X';
			sprintf(plcmntUpdt, "$PLYPMT,%i,%i,%i,%i,%c", TTTADDY, CONTRADDR, ttt285.rowSelect, ttt285.columnSelect, 'X');          //loading string values onto plcmntUpdt command string 
			sprintf(plcmntUpdt, "%s,%i^ \0", plcmntUpdt, calcCheckSum(plcmntUpdt));
			pic.printf("%s", plcmntUpdt);
			plcmntRdy = TRUE;
			msgCount = 0;
		}
		else if (ttt285.gameTtt.turnCounter % 2 == 1)
		{
			ttt285.gameTtt.playerNo = FALSE;
			ttt285.gameTtt.ticTacToe[ttt285.rowSelect][ttt285.columnSelect] = 'O';
			sprintf(plcmntUpdt, "$PLYPMT,%i,%i,%i,%i,%c \0", TTTADDY, CONTRADDR, ttt285.rowSelect, ttt285.columnSelect, 'O');         //loading string values onto plcmntUpdt command string
			sprintf(plcmntUpdt, "%s,%i^\0", plcmntUpdt, calcCheckSum(plcmntUpdt));
			pic.printf("%s", plcmntUpdt);
			plcmntRdy = TRUE;
			msgCount = 0;
		...
		ttt285.gameTime.turnSec = 30;               //setting turntime flag on to reset turntime
...
Date:       05/12/2020
Modified:   NONE
Desc:       function to be attached to the ticker for counting 3 seconds of button press or timer for string messages
Input:      None
Returns:    None
**********************************************************************************************************************/
void pressTimer()
{
	...
	if (plcmntRdy == TRUE || messageRcvd == TRUE)
	{
		msgCount++;
	}
} // eo pressTimer
...
void updateDisplay()
{
	...
	if (ttt285.gameTtt.turnCounter == 0)             //CONDITION FOR READY STATUS
	{
		...
		pc.printf("\e[1;55H Status: READY\t  ");

	}
	if (ttt285.gameTtt.turnCounter > 0 && ttt285.gameTtt.turnCounter <= 9 && ttt285.gameTtt.winFlag != TRUE)     //CONDITION FOR PLAYING STATUS
	{
		ttt285.statusFlag[1] = TRUE;
		pc.printf("\e[1;55H Status: PLAYING\t  ");
		ttt285.statusFlag[0] = FALSE;
	...
	{
		pc.printf("\e[1;55H Status: PLAYER X WINS!!\t");
		pc.printf("\e[4;50H TurnTime: %02d : %02d", ttt285.gameTime.min, ttt285.gameTime.sec);
		ttt285.statusFlag[1] = FALSE;
	...
	if (ttt285.statusFlag[3] == TRUE)
	{
		...
		pc.printf("\e[1;55H Status: PLAYER O WINS!!\t");
		pc.printf("\e[4;50H TurnTime: %02d : %02d", ttt285.gameTime.min, ttt285.gameTime.sec);
		ttt285.statusFlag[1] = FALSE;


	}
	if (ttt285.statusFlag[4] == TRUE)
	{
		pc.printf("\e[1;55H Status: RESET\t");
	}
	if (ttt285.statusFlag[5] == TRUE)
	{
		ttt285.statusFlag[1] = FALSE;
		pc.printf("\e[1;55h Status: DRAW\t");
		ttt285.statusFlag[5] = FALSE;
	}

	pc.printf("\e[2;5H Player: %d \t\t TurnCount: %d", ttt285.gameTtt.playerNo, ttt285.gameTtt.turnCounter);
	if (ttt285.selection == TRUE)       //Condition for row 
	{
		pc.printf("\e[4;5H  ROW\t");
		pc.printf("\e[5;5H Number: %d", ttt285.rowSelect);
	}
	if (ttt285.selection == FALSE)       //Condition for Column
	{
		pc.printf("\e[K\e[4;5H  COLUMN\t");
		pc.printf("\e[5;5H Number: %d", ttt285.columnSelect);
	}
	pc.printf("\e[8;50HCOLUMN");
	pc.printf("\e[9;30H0\t\t1\t\t2");
	pc.printf("\e[10;25H0\t%c |\t %c |\t %c ", ttt285.gameTtt.ticTacToe[0][0], ttt285.gameTtt.ticTacToe[0][1], ttt285.gameTtt.ticTacToe[0][2]);
	pc.printf("\e[11;20HR ------------------------------------------------------------");
	pc.printf("\e[12;25H1\t%c |\t %c |\t %c ", ttt285.gameTtt.ticTacToe[1][0], ttt285.gameTtt.ticTacToe[1][1], ttt285.gameTtt.ticTacToe[1][2]);
	pc.printf("\e[14;20HW ------------------------------------------------------------");
	pc.printf("\e[15;25H2\t%c |\t %c |\t %c ", ttt285.gameTtt.ticTacToe[2][0], ttt285.gameTtt.ticTacToe[2][1], ttt285.gameTtt.ticTacToe[2][2]);
	...
	if (plcmntRdy == TRUE)       //condition to check if player placement has been made 
	{
		...
		{
			pc.printf("\e[18;25H%s", plcmntUpdt);
		...
		if (msgCount >= 5)
		{
			...
			msgCount = 0;
			pc.printf("\e[18;25H                                                                       ");          // else print nothing 
			plcmntRdy = FALSE;
	...
	if (messageRcvd == TRUE)
	{
		...
		if (msgCount >= 5)
		{
			...
			msgCount = 0;
			pc.printf("\e[22;25H                                                                       ");          // else print nothing 
			messageRcvd = FALSE;
	...
	if (autoPlcmnt == TRUE)
	{
		autoPlcmnt = FALSE;
		while (autoPlcmnt != 0);
		ttt285.selection = !ttt285.selection;
		ttt285.gameTtt.turnCounter++;
...
int main()
{
	secondCounter.attach(&pressTimer, 1.0);
	initializeTttSys();
	pic.attach(&receiveByte, Serial::RxIrq);
	while (ttt285.gameTtt.turnCounter < 10)
	{
		...
		if (pbIncrement == FALSE)
		{
			increment();
		}
		if (pbDecrement == FALSE)
		{
			decrement();
		}
		if (pbMode == FALSE && pbEnter == FALSE)
		{
			gameRst = TRUE;
			if (resetCount >= 3)
			{
				resetCount = 0;
				sprintf(gameReset, "$GAMERST,%i,%i,%c \0", TTTADDY, CONTRADDR, 'Y');
				sprintf(gameReset, "%s,%i^\0", gameReset, calcCheckSum(gameReset));
				pic.printf("%s", gameReset);
				pc.printf("\e[19;25H%s", gameReset);
				initializeTttSys();
				gameRst = FALSE;
			}
		}
		updateDisplay();
	} // eo while(turnCounter)
} // eo main
